...
options = odeset('RelTol',1e-9,'AbsTol',[1e-9 1e-9 1e-9 1e-9 1e-9 1e-9 1e-9 1e-9 1e-5],'MaxStep',0.0005);
t = 25;%25
tspan = linspace(0,t,1000*t);
k = 0.9;
u = 0.54;

[T,Y1] = ode45(@(t,y) model_1_eth(t,y,u,k),tspan,[0 0 0 0 0 0 0 0.00092 400],options);


P = zeros(1000*t,1);
for i = 1:t*1000
    P(i) = 1000*(Y1(i,5)/18+Y1(i,6)/32+Y1(i,7)/44+Y1(i,8)/28)*8.314*Y1(i,9)/0.001;%Y(i,3)/34
end
